...
Your Task:
1. Analyze the `network_security_log.t./xt` to understand the past issues.
...
logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s', level=logging.INFO, filename='Troubleshooting_studentID.log')
...
def validate_username(username):
    ...
    logging.info("username assertion is a string and not empty -> passed")
...
def login():
    if request.method == 'POST':
        ...
        if not (ip_address.startswith("192.168.") or ip_address.startswith("10.")):
            logging.warning(f"Suspicious IP Address: {ip_address}")
...
def rent_equipment():
    ...
    if request.method == 'POST':
        ...
        logging.info(f"Renting {equipment_type} for {days_str} days.")
        ...
        try:
            ...
            logging.warning(f"Equipment must be one of the options {e}")

        except ValueError as e:
            logging.error(f"Day must be a number {e}")
        
        except AssertionError as e:
            logging.info(f"Day must be a positive number! {e}")
