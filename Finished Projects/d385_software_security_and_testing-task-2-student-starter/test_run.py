...
def test_general_vuln_1_hardcoded_secrets_remediated():
    ...
    assert app.secret_key != "supersecretkeyforflasksessions", \
    "secret_key is still hard coded"
    
    print("Hard coded secret key test: Pass")
...
def test_general_vuln_2_plaintext_passwords_remediated():
    ...
    for username, data in USERS_DB.items():
        password_list = [
            "admin123", "alice456", "bob789", "charlie000"
        ]
        parts = data["password"].split(":")

        assert len(parts) == 2, \
            f"User {username} does not have to parts (salt:hash)"
        assert len(parts[0]) > 0 and len(parts[1]) > 0, \
            f"User {username} parts are empty; cannot be empty (str(salt) > 0 : str(hash) > 0)"
        
        assert data["password"] not in password_list, \
            f"User {username} still has plaintext password"
