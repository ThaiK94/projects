...
API_KEY = os.environ.get("API_KEY", "sk_live_abc123xyz789secretkey")
...
# print(app.secret_key, "test test test")
# =============================================================================
...
#function for hashing password using pbkdf2_hmac
def hash_password(pwd):
    ...
    # call the hashlib.pbkdf2_hmac(...) function returns a hash object
    d = hashlib.pbkdf2_hmac("sha256", enc_pwd, salt, 100)

    

    return (salt.hex()+":"+d.hex())

def dehash(original_pwd, user_input):
    salt_hex, hash_hex = original_pwd.split(":")
    salt_value = bytes.fromhex(salt_hex)

    enc_pwd = user_input.encode()
    
    # call the hashlib.pbkdf2_hmac(...) function returns a hash object
    d = hashlib.pbkdf2_hmac("sha256", enc_pwd, salt_value, 100)
    return (salt_value.hex()+":"+d.hex())
    

USERS_DB = {
    "admin": {"password": hash_password("admin123"), "role": "admin", "api_key": "sk_admin_key123"},
    "alice": {"password": hash_password("alice456"), "role": "user", "api_key": "sk_alice_key456"},
    "bob": {"password": hash_password("bob789"), "role": "user", "api_key": "sk_bob_key789"},
    "charlie": {"password": hash_password("charlie000"), "role": "guest", "api_key": "sk_charlie_key000"}
}

#testing
# for name, info in USERS_DB.items():
#     print(name, info.get("password"), " test test test")
...
def require_api_key():
    ...
    try:
        ...
        print("api authorization failed ", parts)
        return None
...
def require_role(username, required_role):
    ...
    try:
        ...
        if user_role not in role_rank or required_role not in role_rank:
            return False
        
        if role_rank.get(user_role) >= role_rank.get(required_role):
            return True
        else:
            return False
...
def authenticate():
    ...
    if username in USERS_DB:
        ...
        if USERS_DB[username]['password'] == dehash(USERS_DB[username]['password'] ,password):
            return jsonify({
                "status": "success",
...
def login():
    if request.method == 'POST':
        ...
        print(USERS_DB['admin']['password'])
        if username in USERS_DB and USERS_DB[username]['password'] == dehash(USERS_DB[username]['password'] ,password):
